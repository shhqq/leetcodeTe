...
187 本质是存储中间结果，并进行查找。
无论是使用set还是map，本质都是==hash查找==。但是不能使用List.contains()，String.indexOf()之类的函数，因为这种函数是使用==遍历查找==，耗时比较长。

199 就是树的广度优先遍历，每层最后一个保存下来即可。

==200== 两种方法：

1. 使用深度优先(depth first search)，这样形式简单，时间也比较快。
2. 使用栈，保存当前边界向外扩展，类似于广度优先，形式复杂一些，实际的时间也比较长。

==201== 这个确实看不懂，没办法了。

## 20201009 总结

==207== ==图==的相关知识。即**有向图**中是否存在环，使用BFS或者DFS进行搜索。

1. 首先这就是判断这些课程是否构成了一个**有向无环图**(Directed Acyclic Graph, DAG).
2. 对于DAG的判断，首先想到的就是==拓扑排序==(topological sort)
3. 有向无环图的拓扑排序是指：对图进行拓扑排序即求其中节点的一个拓扑序列，使得对于所有的有向边(U, V)(由U指向V的一条边)，在该序列中节点U都排在节点V之前。
4. 有向无环图有一个特性：

```
    如果是有向无环图，那么一定存在至少一个==入度==为0的节点，将该节点删除，并将该节点的所有==出度==都删除，那么剩下的图还是一个有向无环图。
```

5. 根据这个特性，有两种方式做拓扑排序，一种是广度优先(BFS)：每次都找到一个入度为0的节点，并将该节点入度改为-1，同时，将该节点指向的所有节点的入度都-1（相当于删除了该节点）。这样如果可以循环N（N为图节点数目）次，则是有向无环图；如果不到N，就找不到入度为0的节点了，则不是有向无环图，即BFS；
6. 一种是深度优先(DFS): 从每个节点出发，向下遍历，并标记已经通过的节点和当前正在遍历的节点。已经通过的节点表示从当前节点出发，没有出现环。

[Reference1](https://blog.csdn.net/fuxuemingzhu/article/details/82951771)



==208== 一个全新的数据结构，字典树，或单词查找树。利用一个新的节点结构。再进一步，可以看211题。

209 这个不难，注意利用中间结果。

## 20201010

210 有了207的基础，210就容易了，使用BFS，每次都跳出入度为0的节点，然后将这些节点入度改为-1；然后将这些节点的下一级的节点入度都减1。

如果还没有遍历完，就找不到入度为0的节点了，说明存在环。

211 这个是在208基础上，增加了一个模糊查询。使用递归即可。

==198、213== 是相似的题目，都是考察**动态规划**

1. 一般来说，这种题目首先想到使用==递归==；
2. 递归过程中可能会有一些重复计算，如s(i) = f(s(i-1), s(i-2))等，可以使用一个数组来记录一些中间结果。
3. 递归就会有额外的内存消耗，有时候可以使用==迭代==来规避。
    * 迭代避免递归的额外内存消耗，但可能形式更复杂。
    * 递归可能形式简单，但内存消耗大
4. 如果数组只需要用到有限个值，那么可以使用有限个变量来代替数组。

就本题而言，递归和迭代的形式都不复杂，因此都可以。

213题是首尾相连，分为两次即可：一次包含首不含尾，一次不含首含尾。

[Reference](https://leetcode.com/problems/house-robber/discuss/156523/From-good-to-great.-How-to-approach-most-of-DP-problems.)

## 20201014

==215== 基本上是一个排序题。如果直接使用sort函数，也可以；第二种就是使用选择或冒泡排序，但是当k比较大时，耗时长；第三种就是使用快速排序：选择一个基准e，将数组重新整理，使得e左边的都<=e，右边的都>e，然后看e的编号和k比较，若index(e) = k，则返回e；若index(e) > k，则递归左侧；若index(e) < k，则递归右侧。

216，使用backTracking，也没什么更好的方式，基本就是暴力方法。

## 20201016

==220== 使用桶排序的方法，也就是划段，每段宽度为t+1（避免t为0）。如果存在某一个段，有两个数，那么这两个数的差的绝对值，一定小于t。还要判断是否存在相邻的段，也可能存在满足|a-b| <=t的情况。

划窗的宽度为k，当桶内元素达到k个时，需要去除最左侧的元素。

还要注意越界的问题，转化为long类型。

[Reference](https://jerring.github.io/2019/05/24/leetcode-00220/)

==221== 寻找矩阵中最大的全1的方形。
1. 可以使用遍历的方式；
2. 使用dp，但是这个关系不是很好找；
3. 优化dp：**当使用dp过程中保存了中间结果时，考虑中间结果是否可以简化，并不需要保存所有的中间结果。**

==222= 完全二叉树的节点个数

1. 使用遍历方法，最简单的是BFS，直接统计个数；
2. 使用递归，左子树个数+右子树个数+1；
3. 利用上完全二叉树的性质，改进递归；

223 两个矩形去掉重叠部分的面积和。有点意思。
